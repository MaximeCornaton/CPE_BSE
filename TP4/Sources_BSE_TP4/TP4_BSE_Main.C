...
void Cfg_CLK_UART0(void)
{	
	TCON &= ~(1<<6); //Timer1 Stoppé (précaution)
	TCON &= ~(1<<7);//Flag Timer1 effacé
	CKCON |= (1<<4); //Config CKCON - | T1M: Timer1 ClockSelect – CLK Timer = Sysclk
	
	TMOD |=(1<<5);//Config TMOD - Timer1 configuré en timer 8 bit avec auto-reload
	TMOD &= ~(1<<4);//Config TMOD - Timer1 configuré en timer 8 bit avec auto-reload
	
	TH1 = 0xBB;//Programmation du registre de rechargement TH1
	//Initialisation du registre Timer TL1 – Nécessaire? /Non car ar defaut à 0.
	IE &= ~(1<<3); //Dévalidation de l’interruption Timer1 – Action sur registre IE
	TCON |= (1<<6); //Timer1 démarré - – Action sur TR1 de TCON
}
